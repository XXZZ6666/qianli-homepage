...
var DEPLOYED_API ='https://13b35d36.r1.cpolar.top';  // 例：'https://xxxx.cpolar.io' 或 'https://你的服务器域名'

function apiBase() {
    var host = window.location.hostname || '';
    var protocol = window.location.protocol;
    // 本地 / 局域网打开页面，直接连本地 3000（不走 cpolar 公网隧道）
    if (protocol === 'file:' || host === 'localhost' || host === '127.0.0.1' || host === '::1' || host.startsWith('192.168.') || host.startsWith('10.')) {
        return 'http://localhost:3000';
    }
    // 配置了固定 API 地址就用它（GitHub Pages / 公网托管场景）
    if (DEPLOYED_API) {
        return DEPLOYED_API.replace(/\/$/, '');
    }
    var httpProto = protocol.replace(':', '');
    return httpProto + '://' + host + ':3000';
}
...
function bindLoginForm() {
    ...
    async function doLogin(username, password, force) {
        ...
        if (btn) { btn.disabled = true; var oldVal = btn.value; btn.value = '登录中...'; }
        try {
